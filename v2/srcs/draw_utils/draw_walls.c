...
/*   Updated: 2023/07/22 16:51:25 by hmaciel-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	apply_wall_texture(t_root *game, char tex_wall, int col)
{
	if (game->ray.side == 0 && game->ray.ray_dir_x > 0 && tex_wall == '1')
	...
	else if (game->ray.side == 0 && game->ray.ray_dir_x < 0 && tex_wall == '1')
		my_mlx_pixel_put(&game->background, col, game->calc_wall.wally, \
...
void	draw_walls(t_root *game, int col)
{
	...
	tex_wall = game->map[game->calc_dda.map_x][game->calc_dda.map_y];
	while (game->calc_wall.wally < game->calc_wall.draw_end)
	{
		...
		game->calc_wall.texpos += (1.0f * IMGSIZE / \
			game->calc_wall.line_height);
		apply_wall_texture(game, tex_wall, col);
